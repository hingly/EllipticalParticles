...
geom.NumPoints=input_list(10);           
%       NumPoints: number of points around the ellipse

material.E_m=input_list(4);             
%       E_m: matrix Young's modulus [MPa]
material.nu_m=input_list(5);         
%       nu_m: matrix Poisson's ratio [-]
material.sigmax=input_list(6);        
%       sigmax: maximum cohesive strength [MPa]
material.delopen=input_list(7);      
%       delopen: critical normal opening displacement [mm]
material.cohscale=input_list(8);      
%       cohscale: cohesive scaling parameter [-]
material.plstrain=input_list(9);      
%       plstrain: = 1 for plane strain, = 0 for plane stress

loads.NumModes=input_list(11);       
%       NumModes: number of modes in the Fourier sum (must be even)
loads.SigmaBarRatio=input_list(12);   
%       SigmaBarRatio: ratio of SigmaBar2 to SigmaBar1
loads.AppliedLoadAngle=input_list(13); 
%       AppliedLoadAngle: angle of sigbar1 relative to major axis of ellipse
loads.timesteps=input_list(14);        
%       timesteps: number of timesteps
loads.MinimumStrain=input_list(15);    
%       MinimumStrain: minimum applied strain [-]

loads.LoadFactor=input_list(16);      
%       LoadFactor: fraction of maxstrain/minstrain
post.ScalingParameter=input_list(17);  
%	 ScalingParameter: scaling for displaced shape [-]
...
geom.rho=1;                                     
% radius at which we want to calculate stresses - usually 1 for the unit circle
